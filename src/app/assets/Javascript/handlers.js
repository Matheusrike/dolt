...
function checkboxChange(groupId, taskId, checkboxId) {
...
function mainBasicStructure(group_id) {
    ...
    //Seleciona e limpa a main
    const main = document.getElementsByTagName('main')[0];
    main.innerHTML = '';

    // Cria o container do formulário e adiciona na main
    const form_container = document.createElement('div');
    form_container.className = 'form-container';
    main.appendChild(form_container);

    // Cria o formulário, adiciona no container e impede o envio do formulário pela via padrão
    ...
    task_create_form.addEventListener('submit', (event) => {
        event.preventDefault();
    })

    //Adiciona o input no formulário e define o id com base no grupo passado
    const input_container = document.createElement('div');
    input_container.className = 'input-container';
    input_container.innerHTML = `
        <input type="text" class="input" name="new-task" required minlength="1">
        <label for="new-task">Nova tarefa</label>
    `;
    task_create_form.appendChild(input_container);
    const input = input_container.querySelector('input');
    input.id = `new-task-${group_id}`;

    // Cria o botão de submit, adiciona no formulário e adiciona o EventListener para criar a tarefa
    ...
    // Cria um container onde vai ser carregado a lista de tarefas ou o alerta 
    const main_container = document.createElement('div');
    main_container.id = 'main-container';
    main_container.className = 'main-container';
    main_container.innerHdTML = `<ul class="tasks-list demo-list-control mdl-list" id="tasks-list"></ul>`;
